...
@MainActor final class AppSession: ObservableObject {
    ...
    func login(email:String,password:String) async {
        loading=true; error=""
        do { try await api.signIn(email:email,password:password); let docs=try await api.query(collection:"users"); let mine=docs.first{($0["email"] as? String)?.lowercased()==email.lowercased() || ($0["id"] as? String)==api.uid}; role=UserRole(rawValue:(mine?["role"] as? String ?? "UNKNOWN").uppercased()) ?? .unknown; userName=mine?["name"] as? String ?? email; loggedIn=true; await refresh() } catch { error=(error as? FirebaseError).map{ if case let .message(x)=$0{return x}; return "تعذر تسجيل الدخول" } ?? "تعذر تسجيل الدخول" }; loading=false
    }
    func logout(){api.signOut(); loggedIn=false; orders=[]; role = .unknown}
    func refresh() async { guard loggedIn else{return}; loading=true; do { let docs=try await api.query(collection:"orders"); orders=docs.compactMap(Self.order) } catch { error="تعذر تحميل الأوردرات" }; loading=false }
    ...
    func save(_ o:Order) async { do { var f:[String:Any]=["orderNumber":o.orderNumber,"customerName":o.customerName,"agent":o.agent,"phone":o.phone,"secondPhone":o.secondPhone,"region":o.region,"device":o.device,"faultType":o.faultType,"status":o.status,"dateAdded":Int(o.dateAdded),"completionDate":Int(o.completionDate),"maintenanceFee":o.maintenanceFee,"urgent":o.urgent,"photos":o.photos,"videos":o.videos,"customerLocation":o.customerLocation,"notes":o.notes]; try await api.set(collection:"orders",document:o.id.isEmpty ? UUID().uuidString : o.id,fields:f); await refresh() } catch { error="تعذر حفظ الأوردر" } }
}
...
struct AddOrderPage: View {
    @EnvironmentObject var session: AppSession
    var onSaved: () -> Void
    @State private var customer="", agent="", phone="", second="", region="", device="", fault="", notes="", location=""
    @State private var urgent=false, saving=false
    var body: some View {
        NavigationStack {
            Form {
